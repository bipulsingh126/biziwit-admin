  ...
  const saveBlog = async () => {
    try {
      ...
      if (mainImageFile && savedBlog._id) {
        try {
          // Upload the image - this automatically updates the blog record on the server
          await uploadImage(savedBlog._id)
          // REDUNDANT: await api.updateBlog(savedBlog._id, { mainImage: imageUrl })
        } catch (imageErr) {
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="relative flex-1 min-w-64">
              <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 transition-colors ${loading ? 'text-blue-500 animate-pulse' : 'text-gray-400'
                }`} />
              <input
                type="text"
                ...
                disabled={loading}
                className={`pl-10 pr-10 py-2 w-full border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${searchTerm ? 'bg-blue-50 border-blue-300' : 'bg-white'
                  } ${loading ? 'opacity-50' : ''}`}
              />
              {searchTerm && (
                <button
