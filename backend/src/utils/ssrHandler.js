      ...
      if (firstSegment === "ReportStore" && segments[1]) {
        ...
        if (report) {
          seoData = {
            ...
            canonical:
              report.canonical ||
              `https://www.bizwitresearch.com/ReportStore/${report.slug}`,
        ...
        const seoPage = await SEOPage.findOne({ url: firstSegment });
        console.log("From Seo DataBase" ,seoPage);
        if (seoPage) {
          seoData = {
            ...
            canonical:
              seoPage.canonical ||
              `https://www.bizwitresearch.com/${firstSegment}`,
            robots: "index, follow",
