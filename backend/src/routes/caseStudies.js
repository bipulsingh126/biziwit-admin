...
router.get("/stats/overview", authenticate, async (req, res) => {
  try {
    ...
    const publishedCaseStudies = await CaseStudy.countDocuments({
      status: "published",
    });
    const draftCaseStudies = await CaseStudy.countDocuments({
      status: "draft",
    });
    const featuredCaseStudies = await CaseStudy.countDocuments({
      featured: true,
    });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create a new entry in seo management
      const seoData = {
        pageName: "Case Studies",
        titleMetaTag: savedCaseStudy.titleTag,
        url: savedCaseStudy.slug,
        metaDescription: savedCaseStudy.metaDescription,
        keywords: savedCaseStudy.keywords,
      };
      await SEOPage.create(seoData);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      let oldCaseStudySlug = caseStudy?.slug;
      // Update the case study
      Object.assign(caseStudy, req.body);
      const UpdatedCaseStudy = await caseStudy.save();
      const seoData = {
        titleMetaTag: UpdatedCaseStudy.titleTag,
        url: UpdatedCaseStudy.slug,
        metaDescription: UpdatedCaseStudy.metaDescription,
        keywords: UpdatedCaseStudy.keywords,
      };
      // Update related SEO Page
      await SEOPage.findOneAndUpdate({ url: oldCaseStudySlug }, seoData, {
        new: true,
        runValidators: true,
      });
      res.json(caseStudy);
    ...
    try {
      ...
      if (caseStudy.mainImage) {
        const imagePath = path.join(
          ...
          "uploads",
          "case-studies",
          path.basename(caseStudy.mainImage),
        );
        if (fs.existsSync(imagePath)) {
          fs.unlinkSync(imagePath);
        }
      ...
      await SEOPage.findOneAndDelete({url: oldCaseStudySlug})
      res.json({ message: "Case study deleted successfully" });
    } catch (error) {
      console.error("Error deleting case study:", error);
      res.status(500).json({ error: "Failed to delete case study" });
...
router.post(
  ...
  authenticate,
  requireRole("super_admin", "admin", "editor"),
  caseStudyUpload.single("file"),
  async (req, res) => {
    try {
      console.log(
        "🚀 Starting case study image upload for slug:",
        req.params.slug,
      );
