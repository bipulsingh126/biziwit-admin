...
router.post("/", async (req, res, next) => {
  try {
    ...
    const seoData = {
      pageName: "MegaTrends",
      titleMetaTag: doc.metaTitle,
      url: doc.slug,
      metaDescription: doc.metaDescription,
      keywords: doc.metaKeywords.join(", "),
      author:doc?.author || ""
    };
...
router.patch("/:id", async (req, res, next) => {
  try {
    const body = { ...req.body };
    // find megatrend by id
    const oldMegaTrendData = await Megatrend.findById(req.params.id)
    console.log("oldMegaTrendData", oldMegaTrendData)
    const oldMegaTrendSlug = oldMegaTrendData?.slug || "";
    console.log("OldMegaTrendSLug", oldMegaTrendSlug)
    if (body.title && !body.slug) body.slug = await uniqueSlug(body.title);
    const updated = await Megatrend.findByIdAndUpdate(req.params.id, body, {
      new: true,
    });
    if (!updated) return res.status(404).json({ error: "Not found" });
    console.log("Updated Data",updated)
    const SeoData = {
      titleMetaTag:updated.metaTitle,
      url: updated.slug,
      metaDescription:updated.metaDescription,
      keywords:updated.metaKeywords.join(", ")
    }
    const updatedSeo = await SEOPage.findOneAndUpdate({url:oldMegaTrendSlug}, SeoData, {new:true, runValidators:true})
